...
import com.LOGamez.frogger.Frogger;
...
public final class Sound implements LineListener {   
    ...
    */    
    public Sound(String filename, double vol){
        ...
        Clip oldSnd = Frogger.sndMap.get(filename);
        if(oldSnd != null){
            this.aClip = oldSnd;
        } else {
            try{
                System.out.println("Loading Sound: "+Frogger.soundCount+": " + audioFileName);
                ...
                setVolume(vol);
                Frogger.sndMap.put(filename, aClip);
                Frogger.soundCount++;
            } catch(Exception e) {
            }
        }
    ...
    */    
    public Sound(String filename){
        ...
        AudioClip oldAudio = Frogger.audioMap.get(filename);
        if(oldAudio != null){
            this.clip = oldAudio;
        } else {
            try{
                System.out.println("Loading Audio: "+Frogger.audioCount+": "+"/snd"+ filename);
                clip = Applet.newAudioClip(Sound.class.getResource("/snd"+filename));
                Frogger.audioMap.put(filename, clip);
                Frogger.audioCount++;
            } catch (Exception e) {
            }
        }
    ...
    protected void finalize() throws Throwable {
        if(!fileName.isEmpty()){ 
            Frogger.sndMap.remove(fileName);
            System.out.println("Removing Sound from memory: " + fileName);
        }
        if(!audioFileName.isEmpty()){ 
            Frogger.audioMap.remove(audioFileName);
